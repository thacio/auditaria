...
export class CodeAssistServer implements ContentGenerator {
  ...
  // AUDITARIA_GEMINI_EMBEDDINGS: Stub — embeddings use Google AI Studio API key directly,
  // not CodeAssist. See packages/cli/src/core/initializer.ts for the actual implementation.
  async embedContent(
    _req: EmbedContentParameters,
  ): Promise<EmbedContentResponse> {
    throw new Error(
      'CodeAssist does not support embedContent. ' +
      'Use a Gemini API key with provider: "gemini" in knowledge-base.config.json instead.',
    );
  }
