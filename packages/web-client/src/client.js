...
class AuditariaWebClient {
    ...
    showConfirmationDialog(confirmationData) {
        ...
        // Add confirmation details based on type
    ...
    createConfirmationButtons(confirmationData) {
        ...
        buttons.forEach((buttonConfig, index) => {
            ...
            button.onclick = () => this.handleConfirmationResponse(confirmationData.callId, buttonConfig.outcome);
            
            // Add keyboard navigation
            button.addEventListener('keydown', (e) => {
                if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
                    e.preventDefault();
                    const nextButton = buttons[index + 1] ? container.children[index + 1] : container.children[0];
                    nextButton.focus();
