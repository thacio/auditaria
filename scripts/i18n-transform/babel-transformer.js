        ...
        enter(path) {
          ...
          if (hasNestedTextComponents(path.node)) {
            ...
            if (result) {
              ...
              if (debug) {
                debugLogger.debug(`Transformed nested Text component in ${filePath}`);
              }
              return;
            }
            ...
            if (selectiveResult) {
              ...
              const nodeToReplace = isObjectResult ? selectiveResult.node : selectiveResult;
              ...
              if (isObjectResult && selectiveResult.trueBranchKey) {
                ...
                if (selectiveResult.falseBranchKey) {
                  ...
                  transformations.push({
                    type: selectiveResult.hasParams ? 'I18nTextWithParams' : 'I18nText',
                    original: selectiveResult.trueBranchKey,
                    transformed: `I18nText(${selectiveResult.trueBranchKey.slice(0, 50)}...)`,
                    line: path.node.loc?.start?.line || 0,
                  });
              ...
              if (debug) {
                debugLogger.debug(`Selectively transformed mixed content in ${filePath}`);
              }
              return;
            }
          }
          ...
          if (hasMixedStaticDynamicContent(path.node)) {
            ...
            if (mixedResult) {
              ...
              if (debug) {
                debugLogger.debug(`Transformed mixed static/dynamic content in ${filePath}`);
              }
              return;
            }
          }
          ...
          if (paramResult) {
            ...
            if (debug) {
              debugLogger.debug(`Transformed parameterized Text in ${filePath}`);
            }
            return;
          }
...
// Helper: Check if node is a console call
...
// Helper: Transform console call
function transformConsoleCall(node) {
  // Only transform if first argument is a string literal
  if (node.arguments.length > 0 && t.isStringLiteral(node.arguments[0])) {
    ...
    // Replace first argument with t() call
    const newArgs = [tCall, ...node.arguments.slice(1)];

    return {
      node: t.callExpression(node.callee, newArgs),
      original: text,
    };
  }
...
function transformNestedTextComponent(node) {
  ...
  const componentProperties = Object.entries(components).map(
    ([tagName, componentNode]) => {
      return t.objectProperty(t.identifier(tagName), componentNode, false, false);
    },
  );
...
function transformSimpleTextChild(node) {
  ...
  for (const child of node.children) {
    ...
    } else if (t.isJSXElement(child) && child.openingElement?.name?.name === 'Text') {
...
function transformChildrenSelectively(node) {
  ...
  if (strategy.strategy === 'skip') {
    debugLogger.debug(`transformChildrenSelectively: skipping - ${strategy.reason}`);
    return null;
  }

  // Strategy: branched_i18ntext or branched_i18ntext_with_params
  // (ternary + nested Text, with or without variables)
  if (strategy.strategy === 'branched_i18ntext' || strategy.strategy === 'branched_i18ntext_with_params') {
  ...
  if (strategy.strategy === 'i18ntext_with_params') {
    ...
    if (built.templateString && built.templateString.length >= 2 && /[a-zA-Z]/.test(built.templateString)) {
  ...
  function flushStaticGroup() {
    ...
    for (const item of staticGroup) {
      ...
      } else if (t.isJSXExpressionContainer(item) && t.isStringLiteral(item.expression)) {
        textContent += item.expression.value;
      } else if (t.isJSXElement(item) && item.openingElement?.name?.name === 'Text') {
    ...
    if (staticGroupHasTranslatableText && trimmedText.length >= 2 && /[a-zA-Z]/.test(trimmedText)) {
...
function flattenNestedTernary(expr, parentConditions = []) {
  ...
  const trueConditions = [...parentConditions, { test: expr.test, negated: false }];
  ...
  const falseConditions = [...parentConditions, { test: expr.test, negated: true }];
...
function isNestedTernarySimple(expr) {
  ...
  return isNestedTernarySimple(expr.consequent) && isNestedTernarySimple(expr.alternate);
}
...
function countNestedTernaryBranches(expr) {
  ...
  return countNestedTernaryBranches(expr.consequent) + countNestedTernaryBranches(expr.alternate);
}
...
function analyzeJSXContent(children) {
  ...
  for (const child of children) {
    if (t.isJSXText(child)) {
      ...
      if (t.isStringLiteral(expr)) {
        ...
        const isNestedTernary = t.isConditionalExpression(expr.consequent) || t.isConditionalExpression(expr.alternate);
        const nestedBranchCount = isNestedTernary ? countNestedTernaryBranches(expr) : 2;
        ...
        const ternary = {
          ...
          node: child,
          isSimple: t.isStringLiteral(expr.consequent) && t.isStringLiteral(expr.alternate),
          isNestedTernary,
          isNestedTernarySimple: isNestedTernary ? isNestedTernarySimple(expr) : false,
          flattenedBranches: isNestedTernary ? flattenNestedTernary(expr) : null,
      ...
      const tagName = componentCounter[tagBaseName] > 1
  ...
  const allTernariesSimple = analysis.ternaries.every(ter =>
    ter.isSimple || (ter.isNestedTernary && ter.isNestedTernarySimple),
  );
  if (analysis.ternaries.length > 0 && !allTernariesSimple) {
    ...
    if (analysis.nestedText.length > 0) {
      const hasJSXBranches = analysis.ternaries.some(ter =>
...
function getTransformStrategy(analysis) {
  const { staticText, variables, ternaries, nestedText, hasComplexDynamic, complexReason, totalBranches } = analysis;
  ...
  const allTernariesSimple = ternaries.every(ter =>
    ter.isSimple || (ter.isNestedTernary && ter.isNestedTernarySimple),
  );
  ...
  if (hasStatic && !hasVars && hasSingleTernary && !hasNested && allTernariesSimple) {
  ...
  if (hasStatic && hasVars && hasSingleTernary && !hasNested && allTernariesSimple) {
    if (ternaries[0].isNestedTernary) {
      return { strategy: 'multi_branched_parameterized_t', branches: totalBranches };
    }
  ...
  if (hasStatic && hasVars && hasSingleTernary && hasNested && allTernariesSimple) {
    if (ternaries[0].isNestedTernary) {
      return { strategy: 'multi_branched_i18ntext_with_params', branches: totalBranches };
    }
  ...
  if (hasStatic && !hasVars && hasMultipleTernaries && !hasNested && allTernariesSimple) {
    return { strategy: 'multi_branched_t', branches: totalBranches };
  }
...
// Legacy function kept for backward compatibility - now uses buildTemplateFromAnalysis
function extractNestedContentWithTernaryBranch(children, useTrueBranch) {
  const analysis = analyzeJSXContent(children);
  const result = buildTemplateFromAnalysis(analysis, children, useTrueBranch);
  return {
 ...
 */
function createI18nTextElement(templateString, components, parentAttributes, params = {}) {
  const attributes = [];
...
function transformNestedTextWithTernary(node) {
  ...
  if (!trueBranch.templateString || trueBranch.templateString.length < 2) return null;
  ...
  if (!falseBranch.templateString || falseBranch.templateString.length < 2) return null;
  ...
  const ternaryExpression = t.conditionalExpression(condition, trueI18nText, falseI18nText);
...
function getParamName(expression, usedNames = {}) {
  ...
  if (t.isIdentifier(expression)) {
    ...
    if (t.isMemberExpression(expression.callee) && t.isIdentifier(expression.callee.property)) {
      ...
      } else if (t.isMemberExpression(expression.callee.object) && t.isIdentifier(expression.callee.object.property)) {
...
function transformParameterizedText(node) {
  ...
  return {
    node: t.jsxElement(node.openingElement, node.closingElement, [newChild], node.selfClosing),
    original: template,
...
function hasMixedStaticDynamicContent(node) {
  ...
  return strategy.strategy === 'branched_t' ||
...
function generateBranchCombinations(ternaries) {
  ...
  const ternaryBranches = ternaries.map((ter, index) => {
    ...
    return [
      { ternaryIndex: index, branchIndex: 0, value: ter.consequent, conditions: [{ test: ter.test, negated: false }] },
      { ternaryIndex: index, branchIndex: 1, value: ter.alternate, conditions: [{ test: ter.test, negated: true }] },
    ];
  });
...
function extractTextWithBranchCombination(children, branchCombination, ternaries) {
  let text = '';
  const params = {};
  const usedNames = {};
...
function transformMixedStaticDynamic(node) {
  ...
  const branchResults = combinations.map(combo => {
    const extracted = extractTextWithBranchCombination(node.children, combo, ternaries);
    const hasParams = Object.keys(extracted.params).length > 0;

    // Validate text
    if (!extracted.text || extracted.text.length < 2 || !/[a-zA-Z]/.test(extracted.text)) {
      return null;
    }

    const tCall = hasParams
      ? createTCallWithParams(extracted.text, extracted.params)
      : t.callExpression(t.identifier('t'), [t.stringLiteral(extracted.text)]);
  ...
  for (let i = branchResults.length - 2; i >= 0; i--) {
    ...
    if (branch.condition) {
      resultExpr = t.conditionalExpression(branch.condition, branch.tCall, resultExpr);
    }
  }
