...
export class WebSocketManager extends EventTarget {
  ...
  handleMessage(message) {
    ...
    if (
      ...
      message.data?.startingSequence !== undefined
    ) {
    ...
    if (message.sequence !== undefined) {
      ...
      // Ephemeral updates share the same sequence counter. Count them
      // when detecting gaps, or every streamed response triggers a resync.
      const isLikelyWrap =
        this.lastReceivedSequence >= Number.MAX_SAFE_INTEGER - 1000000 &&
      ...
      // Replayed messages can arrive behind live updates; never move the
      // cursor backwards and mistake the next live message for another gap.
      if (!isEphemeral) {
        this.lastPersistentSequence = isLikelyWrap
          ? message.sequence
          : Math.max(this.lastPersistentSequence, message.sequence);
      }
      this.lastReceivedSequence = isLikelyWrap
        ? message.sequence
        : Math.max(this.lastReceivedSequence, message.sequence);
      ...
      if (this.processedSequences.size > this.MAX_PROCESSED_SEQUENCES) {
        const sortedSequences = Array.from(this.processedSequences).sort(
          (a, b) => a - b,
        );
        const toRemove = sortedSequences.slice(
          0,
          sortedSequences.length - this.MAX_PROCESSED_SEQUENCES,
        );
  ...
  checkForMissedMessages() {
    // Ephemeral updates also confirm delivery on this ordered connection.
