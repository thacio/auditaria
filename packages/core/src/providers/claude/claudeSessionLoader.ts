...
// Claude Code records TUI slash-command activity as regular `user` entries
// (often WITHOUT isMeta) whose text is bookkeeping markup, not something the
// human said. These pollute the resumed chat view — filter them out.
...
/** True if a user-entry text is Claude Code local-command bookkeeping, not conversation. */
export function isLocalCommandNoise(text: string): boolean {
  const t = text.trimStart();
  return LOCAL_COMMAND_PREFIXES.some((p) => t.startsWith(p));
}
...
function processUserMessage(
  ...
  if (typeof content === 'string') {
    if (isSystemContextText(content) || isLocalCommandNoise(content)) return;
    if (awaitingSummary) {
  ...
  if (awaitingSummary) {
    ...
    if (
      summaryText &&
      !hasToolResult &&
      !isSystemContextText(summaryText) &&
      !isLocalCommandNoise(summaryText)
    ) {
  ...
  for (const block of blocks) {
    ...
    if (block.type === 'text' && block.text) {
      if (isSystemContextText(block.text) || isLocalCommandNoise(block.text)) {
        continue;
      }
