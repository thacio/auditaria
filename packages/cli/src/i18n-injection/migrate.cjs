 ...
 * Node.js version for direct execution
 */
...
function createPattern(enStr, ptStr) {
  ...
  for (const { param } of paramPositions) {
    ...
    // Find parameter in Portuguese string and replace with group reference
  ...
  // Escape special regex characters except capture groups and dots inside them
  pattern = pattern
    .replace(/[.*+?^${}()|[\]\\]/g, (match) => {
      // Don't escape parentheses (for capture groups)
      if (match === '(' || match === ')') {
        return match;
      }
      return '\\' + match;
