...
function extractLlmText(llmOutput) {
  ...
  if (Array.isArray(llmOutput)) {
    ...
    for (const part of llmOutput) {
      ...
      // Gemini parts and Claude {type:'text'} blocks both carry .text
      else if (part.text) {
  ...
  if (
    llmOutput.inlineData ||
    llmOutput.fileData ||
    (llmOutput.type === 'image' && llmOutput.source?.type === 'base64')
  ) {
...
function createToolHeader(tool) {
  ...
  if (tool.llmOutput) {
    ...
    toggleLlmBtn.addEventListener('click', (e) => {
      ...
      const collapsibleContainer = parentItem.querySelector(
...
function createToolOutput(tool) {
  ...
  if (!outputContent && tool.status === 'Executing' && tool.liveOutput) {
    ...
    console.log(
      '[ToolRenderer] Using liveOutput:',
      outputContent.substring(0, 100),
    );
  }
  ...
  console.log('[ToolRenderer] Processing outputContent:', {
    ...
    startsWithBrowserSteps:
      typeof outputContent === 'string' &&
      outputContent.startsWith('{"browserSteps"'),
    preview:
      typeof outputContent === 'string'
        ? outputContent.substring(0, 150)
        : outputContent,
  });

  if (typeof outputContent === 'string') {
    // AUDITARIA_ARTIFACTS: publish result card
    const artifactData = tryParseArtifactDisplay(outputContent);
    if (artifactData) {
    ...
    console.log(
      '[ToolRenderer] tryParseBrowserStepDisplay result:',
      browserStepData,
    );
    if (browserStepData) {
    ...
    if (
      outputContent.browserSteps &&
      Array.isArray(outputContent.browserSteps)
    ) {
...
function isTodoWriteResult(text) {
  return (
    text &&
    text.includes('Todos have been') &&
    text.includes('modified successfully')
  );
}
...
function renderWriteTodosList(todos) {
  ...
  todos.forEach((todo, index) => {
    ...
    contentEl.textContent =
      todo.description || todo.content || `Task ${index + 1}`;
 ...
 */
// AUDITARIA_ARTIFACTS: {"artifact": {...}} sentinel emitted by the artifact tool
function tryParseArtifactDisplay(input) {
  if (typeof input !== 'string' || !input.startsWith('{"artifact"'))
    return null;
  try {
    const parsed = JSON.parse(input);
    return parsed && parsed.artifact && parsed.artifact.id ? parsed : null;
  } catch {
    return null;
  }
}
...
function renderArtifactCard(artifact) {
  ...
  card.className = 'artifact-tool-card';
  const head = document.createElement('div');
  head.className = 'artifact-tool-card-head';
  head.innerHTML = `<span class="artifact-tool-card-verb">${artifact.created ? 'Published' : 'Updated'}</span> <span class="artifact-tool-card-star">✻</span> <span class="artifact-tool-card-title">${escapeHtml(artifact.favicon + ' ' + artifact.title)}</span> <span class="artifact-tool-card-version">v${artifact.version}</span>`;
  card.appendChild(head);
  if (artifact.description) {
    const desc = document.createElement('div');
    desc.className = 'artifact-tool-card-desc';
    desc.textContent = artifact.description;
    card.appendChild(desc);
  }
  const actions = document.createElement('div');
  actions.className = 'artifact-tool-card-actions';
  if (artifact.url) {
    const preview = document.createElement('button');
    preview.className = 'artifact-tool-card-btn';
    preview.textContent = 'Preview';
    preview.addEventListener('click', () => {
      document.dispatchEvent(
        new CustomEvent('auditaria-artifact-open', {
          detail: { id: artifact.id },
        }),
      );
    });
    const open = document.createElement('a');
    open.className = 'artifact-tool-card-btn';
    ...
    note.className = 'artifact-tool-card-note';
    note.textContent = 'Stored; open the web interface to view it.';
...
function renderBrowserSteps(data) {
  ...
  console.log('[ToolRenderer] Checking stream viewer conditions:', {
    ...
    hasExistingViewer: data.sessionId
      ? activeStreamViewers.has(data.sessionId)
      : false,
  ...
  if (data.sessionId) {
    ...
    if (!activeStreamViewers.has(data.sessionId)) {
      try {
        ...
        const streamContainer = createEmbeddedStreamViewer(
          data.sessionId,
          'running',
        );
        ...
        console.log(
          '[ToolRenderer] Created stream viewer for session:',
          data.sessionId,
        );
      } catch (error) {
