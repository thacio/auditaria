...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  it('should use chatty system prompt for preview model', () => {
    vi.mocked(mockConfig.getActiveModel).mockReturnValue(PREVIEW_GEMINI_MODEL);
    const prompt = getCoreSystemPrompt(mockConfig);
    expect(prompt).toContain('You are Auditaria CLI, an interactive CLI agent'); // Check for core content
  ...
  it('should append userMemory with separator when provided', () => {
    ...
    expect(prompt).toContain('You are Auditaria CLI, an interactive CLI agent'); // Ensure base prompt follows
  ...
  it.each([
    ...
    (toolNames, expectCodebaseInvestigator) => {
      ...
      if (expectCodebaseInvestigator) {
        ...
        // The CodebaseInvestigator only affects the "Development Lifecycle" section.
      } else {
        expect(prompt).not.toContain(
          `Utilize specialized sub-agents (e.g., \`codebase_investigator\`) as the primary mechanism for initial discovery`,
        );
