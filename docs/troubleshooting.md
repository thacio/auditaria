...
- **Error: Command not found (when attempting to run Auditaria CLI with `auditaria`).**
  ...
  - **Solution:**
    ...
    - If you installed `auditaria` globally, check that your `npm` global binary directory is in your `PATH`. You can update Auditaria CLI using the command `npm install -g @google/gemini-cli@latest`.
