...
describe('loadClaudeSessionAsContent', () => {
  ...
  it('skips local-command bookkeeping user entries (caveat, command-name, stdout)', async () => {
    // Real shapes from Claude Code 2.1.x JSONL: slash-command records land as
    // regular `user` entries (often WITHOUT isMeta) whose text is markup.
    const result = await parseEntries([
      userText(
