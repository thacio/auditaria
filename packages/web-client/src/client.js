...
import { ProviderTerminalViewer } from './components/ProviderTerminalViewer.js'; // AUDITARIA_PROVIDER_TERMINAL
...
class AuditariaWebClient {
  constructor() {
    ...
    // AUDITARIA_PROVIDER_TERMINAL: Live xterm.js mirror of the active
    // provider's PTY (Claude Code, GitHub Copilot, …). Self-subscribes to
    // provider_pty_state / provider_pty_data on the WS manager and
    // shows/hides its own modal in sync with PTY lifecycle.
    this.providerTerminalViewer = new ProviderTerminalViewer(this.wsManager);
    ...
    this.handleModelMenuViewportChange =
      this.handleModelMenuViewportChange.bind(this);
  ...
  setupKeyboardShortcuts() {
    ...
    this.keyboardManager.register('Escape', () => {
      if (
        this.loadingIndicator.getState().isLoading &&
        this.wsManager.getState().isConnected
      ) {
  ...
  setupInputHandlers() {
    ...
    this.messageInput.addEventListener('keydown', (event) => {
      ...
      if (
        this.slashAutocomplete &&
        this.slashAutocomplete.handleKeyDown(event)
      ) {
  ...
  sendMessage() {
    ...
    if (
      (!message && this.attachments.length === 0) ||
      !this.wsManager.getState().isConnected
    ) {
      return;
    }
    ...
    if (
      this.wsManager.sendUserMessage(
        message,
        isSlashCommand ? [] : this.attachments,
      )
    ) {
  ...
  async handleFileSelection(files) {
    ...
    const existingFiles = new Set(
      this.attachments.map((att) => `${att.name}_${att.size}`),
    );
    ...
    if (uniqueFiles.length > filesToProcess.length) {
      showInfoToast(
        `Added ${filesToProcess.length} of ${uniqueFiles.length} files (max ${maxAttachments})`,
      );
    }
    ...
    for (const file of filesToProcess) {
      try {
        ...
        const attachment = await FileHandler.createAttachment(
          file,
          this.attachments,
        );
    ...
    if (duplicateFiles.length > 0 && addedCount > 0) {
      console.log(
        `Added ${addedCount} files, skipped ${duplicateFiles.length} duplicates`,
      );
    }
  }
  ...
  showClearConfirmation(message) {
    ...
    cancelButton.className =
      'clear-confirmation-button clear-confirmation-cancel';
    cancelButton.textContent = 'Cancel';
    cancelButton.onclick = () => this.hideClearConfirmation();

    const confirmButton = document.createElement('button');
    confirmButton.className =
      'clear-confirmation-button clear-confirmation-confirm';
...
<div class="messages-container">${messagesContainer.innerHTML}</div>
  ...
  formatModelFooterText(rawModel, displayModel) {
    ...
    if (rawLower.startsWith('claude-code:')) {
      ...
      const label =
        variant.toLowerCase() === 'auto' ? 'Auto' : toTitleCase(variant);
      return `Claude (${label})`;
    }
  ...
  hasModelMenuOptions() {
    return (
      this.modelMenuData?.groups?.some(
  ...
  setModelMenuExpanded(expanded) {
    if (this.modelMenuAnchor) {
      this.modelMenuAnchor.setAttribute(
        'aria-expanded',
        expanded ? 'true' : 'false',
      );
    }
  ...
  getCodexEffortStateForSelection(selection) {
    ...
    const nextEffort =
      supportedEfforts[
        Math.min(supportedEfforts.length - 1, currentIndex + 1)
      ] || currentEffort;
  ...
  handleModelMenuOutsideClick(event) {
    ...
    if (
      this.modelMenuElement.contains(target) ||
      this.modelMenuAnchor?.contains(target)
    ) {
      return;
    }
    this.closeModelMenu();
  ...
  updateFooter(footerData) {
    ...
    const workingDirectory =
      footerData.workingDirectory || footerData.targetDir || '';
  ...
  setupHeaderPanelToggles() {
    ...
    if (toggleFilesButton) {
      ...
      this.fileTreePanel.on('collapse-changed', ({ isCollapsed }) => {
        ...
        toggleFilesButton.title = isCollapsed
          ? 'Show File Explorer'
          : 'Hide File Explorer';
      });

      // Set initial state
      toggleFilesButton.classList.toggle(
        'active',
        !this.fileTreePanel.isCollapsed,
      );
    }
    ...
    if (toggleEditorButton) {
      ...
      this.editorPanel.on('visibility-changed', ({ isVisible }) => {
        ...
        toggleEditorButton.title = isActive
          ? 'Hide Editor Panel'
          : 'Show Editor Panel';
      });
  ...
  handleFileOpen(path) {
    ...
    if (isBinaryFile(filename)) {
      ...
      if (
        this.previewManager &&
        this.previewManager.canPreview(language, filename)
      ) {
  ...
  showBinaryFileError(path, filename) {
    ...
    showErrorToast(
      `Cannot open "${filename}". This file type cannot be previewed.`,
    );
  }
  ...
  if (toggleAllRawBtn) {
    toggleAllRawBtn.addEventListener('click', () => {
      ...
      if (isShowingRaw) {
        ...
        document
          .querySelectorAll('.tool-toggle-llm-btn.active')
          .forEach((btn) => btn.click());
      } else {
        toggleAllRawBtn.classList.add('active');
        document
          .querySelectorAll('.tool-toggle-llm-btn:not(.active)')
          .forEach((btn) => btn.click());
      }
    });
  }
