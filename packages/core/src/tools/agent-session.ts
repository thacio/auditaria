...
IMAGE GENERATION & EDITING: Two providers can generate images natively. Neither exposes any size / resolution / aspect-ratio parameter (you can only hint at framing/orientation in the prompt text — no guaranteed pixel dimensions).
- "agy" — RECOMMENDED for images. Native "generate_image" tool that SAVES the result to a file on disk (~1024px JPEG) and reports the path, so the image is actually retrievable afterward. Inputs: a text Prompt, an output filename (ImageName), and optionally UP TO 3 input images for editing/combining passed BY ABSOLUTE PATH (ImagePaths) — so reference-image edits ARE reachable here: just give the sub-agent the file paths (e.g. "edit these images <abs paths> — change only Y"). Does high-fidelity single-attribute edits (change one thing, preserve the rest).
- "codex" — Native "image_gen.imagegen" tool (param: prompt only). Generates a ~1280×1280 image and can edit an ATTACHED image, BUT it returns the image INLINE with NO saved file or path. Via external_agent_session only the sub-agent's TEXT comes back, so a codex-generated image is NOT retrievable by you. Use codex for images only if you don't actually need the resulting file.
For any task where you need the actual image FILE back, spawn an "agy" sub-agent (give it a clear instruction, let it drive generate_image, then use the saved path it reports).
