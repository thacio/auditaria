...
`settings.json` file or by using the `/settings` command in Auditaria CLI.
...
Auditaria CLI. To focus on the interactive shell, press `Tab`. The terminal
...
tools to detect if they are being run from within Auditaria CLI.
