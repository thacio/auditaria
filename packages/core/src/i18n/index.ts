...
import type { SupportedLanguage, TranslationData, TranslationConfig } from './types.js';
  ...
  translate(key: string, fallback: string, params?: Record<string, string | number>): string {
    ...
    if (currentLangData) {
      ...
      if (currentLangData._exactStrings && currentLangData._exactStrings[key]) {
        ...
        return params ? this.interpolateString(exactTranslation, params) : exactTranslation;
      }
    ...
    if (this.config.fallbackLanguage !== this.config.currentLanguage) {
      ...
      if (fallbackLangData) {
        ...
        if (fallbackLangData._exactStrings && fallbackLangData._exactStrings[key]) {
...
export const t = (key: string, fallback?: string, params?: Record<string, string | number>): string => {
  return i18nManager.translate(key, fallback ?? key, params);
};
