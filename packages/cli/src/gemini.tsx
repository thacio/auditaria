...
const InitializingComponent = ({ initialTotal }: { initialTotal: number }) => {
  ...
  const message = t('commands.mcp.connecting_servers', 'Connecting to MCP servers... ({connected}/{total})', {
    connected,
    total,
  });
...
export async function main() {
  ...
  if (argv.promptInteractive && !process.stdin.isTTY) {
    console.error(
      t('cli.errors.prompt_interactive_stdin', 'Error: The --prompt-interactive flag is not supported when piping input from stdin.'),
    );
    process.exit(1);
  }
